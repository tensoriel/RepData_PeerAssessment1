...
for (i in 1:61){
newstep[,i]  <- as.numeric(as.character( data[ ((288*(i-1)+1): (i*288)) , 1] ))
...
plot(intv,  avgstep, type="l",  main="Average Number of Steps per day",
...
for (i in 1:61){ totalstep1[i] <- sum(newstep1[,i])}
...
for (i in 1:61){
newstep2[,i]  <- as.numeric(as.character( data[ ((288*(i-1)+1):(i*288)) , 1] ))
if (weekdays(as.POSIXct(paste(as.character(data[(i*288),2])), format = "%Y-%m-%d"), abbreviate= T) == as.character("Sat")| weekdays(as.POSIXct(paste(as.character(data[(i*288),2])), format = "%Y-%m-%d"), abbreviate= T) == as.character("Sun")){
weeknd[,i] <- newstep2[,i]
}else{
